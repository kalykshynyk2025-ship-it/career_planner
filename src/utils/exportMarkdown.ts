...
## 5. АНАЛИЗ ВАКАНСИЙ (ТРЕБОВАНИЯ, ОБЯЗАННОСТИ И СПОСОБЫ ДОСТИЖЕНИЯ)
*Детальный разбор 7+ желаемых вакансий, проверка владения и способы закрытия гэпов (${state.vacancy_analyses?.length || 0} пунктов)*

${(state.vacancy_analyses || []).length > 0 ? (state.vacancy_analyses || []).map((a, i) => `
### ${i + 1}. **${a.company}** — ${a.vacancyTitle}
- **Требование / Обязанность**: ${a.item}
- **Тип**: ${a.type || 'Требование'}
- **Статус владения**: ${a.status === 'Владею' ? '🟢 Владею (Да)' : a.status === 'Частично' ? '🟡 Частично' : '🔴 Не владею (Нет)'}
- **Способ достижения**: **${a.achievementMethod}**
- **Заметки / План**: ${a.notes || '—'}
`).join('\n') : '_Анализ вакансий еще не проведен._'}
...
export const generateVacancyAnalysisMarkdown = (state: CareerState): string => {
  ...
  return `# ДЕТАЛЬНЫЙ АНАЛИЗ ВАКАНСИЙ (ТРЕБОВАНИЯ И ОБЯЗАННОСТИ)
*Системный разбор выписанных требований из 7+ желаемых вакансий, проверка владения и способы закрытия гэпов*
...
- **Способ достижения**: **${a.achievementMethod}**
- **Заметки / План**: ${a.notes || '—'}
`).join('\n\n---\n') : '_Данные анализа вакансий отсутствуют._'}
`;
};
