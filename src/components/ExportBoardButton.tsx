...
import { Download, FileText, Check, Copy, Printer, Layers, ChevronDown, ExternalLink } from 'lucide-react';
      ...
      {isOpen && (
        <div className="absolute right-0 mt-2 w-72 rounded-2xl bg-[var(--bg-card)] border border-[var(--color-border)] shadow-xl z-50 py-2 space-y-1 text-xs divide-y divide-[var(--color-border)]">
          <div className="px-3 py-1.5 font-bold text-[var(--text-primary)] flex items-center justify-between">
            <span>Экспорт доски & Документа</span>
            <span className="text-[10px] text-blue-500 font-bold bg-blue-500/10 px-1.5 py-0.5 rounded">PDF / Markdown</span>
          </div>

          {/* PDF Direct Export Section */}
          <div className="py-1 space-y-0.5">
            <div className="px-3 py-1 font-semibold text-[10px] uppercase tracking-wider text-[var(--text-secondary)]">
              Печать и PDF Экспорт (Полный дизайн)
            </div>
            ...
            >
              ...
              <span>Печать страницы / Сохранить в PDF</span>
            </button>
          </div>
          ...
          <div className="py-1 space-y-0.5">
            <div className="px-3 py-1 font-semibold text-[10px] uppercase tracking-wider text-[var(--text-secondary)]">
              Красивый Текстовый Экспорт (Markdown)
            </div>

            <button
              onClick={handleDownloadSingle}
              className="w-full text-left px-3 py-2 hover:bg-emerald-500/10 text-[var(--text-primary)] flex items-center space-x-2 cursor-pointer transition-colors"
            >
              <Download className="w-3.5 h-3.5 text-emerald-500" />
              <span>Скачать Markdown этой доски (.md)</span>
            </button>
            ...
            >
              ...
              <span>{copied ? 'Скопировано!' : 'Скопировать текст этой доски'}</span>
            </button>
            ...
            >
              ...
              <span>Скачать все доски по отдельности (.md)</span>
            </button>
          </div>
